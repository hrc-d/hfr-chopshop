...
from gym.envs.box2d.DA_car_dynamics import Car
...
class DACarRacing(gym.Env, EzPickle):
    metadata = {
    ...
    def __init__(self, verbose=1):
        ...
        self.track_filename = "gym/gym/envs/box2d/da_track.pkl"
    ...
    def _create_track(self, save_path=None):
        ...
        if save_path is not None:
            print(f"saving the map to {save_path}")
    ...
    def step(self, action):
        if action is not None:
            self.steer,self.gas,self.brake = action
            self.car.steer(-action[0])
            self.car.brake(action[2]) # I swapped the order to allow the governer on gas action to brake if needed
            self.car.gas(action[1])
            

        else:
            self.steer,self.gas,self.brake = (0.0,0.0,0.0)
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            self.speed_label = pyglet.text.Label('00 MPH', font_size=36,
                x=20, y=WINDOW_H-40, anchor_x='left', anchor_y='center', color=(255,255,255,255))
            self.gas_label = pyglet.text.Label('Gas', font_size=36,
                x=240, y=WINDOW_H-40, anchor_x='left', anchor_y='center', color=(0,0,0,50))
            self.brake_label = pyglet.text.Label('Brake', font_size=36,
                x=360, y=WINDOW_H-40, anchor_x='left', anchor_y='center', color=(0,0,0,50))
        ...
        t.disable()
        self.render_indicators(WINDOW_W, WINDOW_H, mode)
    ...
    def render_indicators(self, W, H, mode):
        ...
        if mode != 'state_pixels':
            self.speed_label.text = '%02i MPH' % np.linalg.norm(self.car.hull.linearVelocity)
            self.speed_label.draw()
            self.gas_label.color = (0,0,0,max(50,int(self.gas*255)))
            self.gas_label.draw()
            self.brake_label.color = (255,0,0,max(50,int(self.brake*255)))
